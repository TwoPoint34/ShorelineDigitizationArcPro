    ...
    if WLVTextension == '.xls' or '.xlsx':  # Make sure that input spreadsheet has proper file extension
        ...
        for row in FGDBsearchcursor:  # Get values below from FGDB table
            ...
            if len(ImageDEMWLTable) != 0:
                imageraster = arcpy.sa.Raster(imagerasterpath)  # If the user inputs an Excel file, access the raster and save to variable
            else:
            ...
            binaryrasterpathlist.append(unsuprasterpath)
            ...
            if len(ThresholdInputString) != 0 and len(str(NIRBandNumber)) !=0 or len(str(SWIRBandNumber)) !=0:
        ...
        # GET BASENAME FOR EACH BINARY RASTER INPUT AND ADD TO OUTPUT FILE NAMES
            ...
            for binaryraster in binaryrasterpathlist:
                ...
                out_name = 'WaterFeatures' + '_' + timestamp + '_' + str(imagecount) + '_' + str(binarycount)
                WaterFeatures = arcpy.conversion.FeatureClassToFeatureClass(Water_Selection, out_path, out_name)
                # The script tool assumes that the largest water body is the water body the user wants the shoreline
                # to be delineated for
                MaxCursor = arcpy.SearchCursor(WaterFeatures)
                value = 0
                ...
                where_clause = 'Shape_Area > ' + MaxWaterIntString + ' AND gridcode = 1'   # Identify largest water polygon as only feature larger
                # than int(MaxWater).  For some reason, the selection of gridcode =1 is lost prior to this step, so I have added gridcode = 1 to
                # the SQL where_clause once more.
                Largest_Water_Selection = arcpy.management.SelectLayerByAttribute(WaterFeatures, "", where_clause)

                out_feature_class = 'LargestWaterFeature' + '_' + timestamp + '_' + str(imagecount) + '_' + str(binarycount)
                LargestWaterFeature = arcpy.management.CopyFeatures(Largest_Water_Selection, out_feature_class)
                ...
                out_feature_class = 'ElimPolyPart' + '_' + timestamp + '_' + str(imagecount) + '_' + str(binarycount)
                WaterPolygonSelection = arcpy.management.EliminatePolygonPart(LargestWaterFeature, out_feature_class, "PERCENT", "", 99, "ANY")
                ...
                arcpy.management.Delete(WaterFeatures)
                arcpy.management.Delete(LargestWaterFeature)
                ...
                try:  # Attempting to dissolve when no line feature is created by a given water/land detection algorithm will result in an
                # error.  The other processing tools after RasterToPolygon and before Dissolve do not appear to throw an error when given
                # a blank input.
            ...
            AddToTOC = arcpy.management.Merge(linefeatures, output, "", "ADD_SOURCE_INFO")  # Adding Source Info retains the
        ...
        # Delete intermediate products from the output folder
            ...
            DEMraster = arcpy.sa.Raster(DEMrasterpath)
            out_polyline_features = imagerasterbasename + '_' + "GroundReferenceWaterLevel" + '_' + AdditionToOutputFileName + '_' + str(rowcount) + '_' + timestamp
            contour_value = [WaterLevel]  # Input value is a list (even if of a single item)
            AddToTOC = ContourList(DEMraster, out_polyline_features, contour_value)
